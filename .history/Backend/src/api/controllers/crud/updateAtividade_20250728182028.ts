...
export async function updateAtividade(req: Request, res: Response) {
  const { id } = req.params;
  // Aceita tanto dueDate (frontend antigo) quanto date (frontend novo)
  let { title, description, status, priority, dueDate, date } = req.body;
  ...
  const dataVenc = date || dueDate;
  if (dataVenc) {
    try {
      prismaDate = new Date(dataVenc);
