...
function getDataVencimento(task: { dueDate?: string; date?: string }) {
  ...
  const fetchTarefas = async () => {
    ...
    try {
      ...
      const res = await api.get('/auth/tarefas', { params: { userId } });
  ...
  return (
    <div className="min-h-screen bg-[#192132] text-white p-2 sm:p-4 md:p-8">
      ...
      <div className="flex flex-col gap-4 sm:gap-6">
        {loading ? (
          ...
          tarefasFiltradas.map((task) => (
            <div
              key={task.id}
            className="bg-[#232c43] p-4 sm:p-6 rounded flex flex-col md:flex-row md:items-center md:justify-between shadow border border-gray-700"
            >
              <div className="flex-1 flex flex-col gap-1 min-w-0">
                ...
                <p className="text-gray-400 text-sm sm:text-base mb-1 break-words">{task.description}</p>
              ...
              <div className="flex gap-2 mt-4 md:mt-0 flex-wrap justify-end">
                <button
                  className="bg-white text-gray-800 p-2 rounded hover:bg-gray-200 border border-gray-300 transition text-xs sm:text-sm"
                  title="Editar"
                  onClick={() => {
                ...
                <button
                  className="bg-red-600 text-white p-2 rounded hover:bg-red-700 border border-red-700 transition text-xs sm:text-sm"
                  title="Excluir"
                  onClick={async () => {
                    if (window.confirm('Tem certeza que deseja excluir esta tarefa?')) {
                      await api.delete(`/auth/tarefas/${task.id}`);
                      fetchTarefas();
                    }
                  }}
                >
