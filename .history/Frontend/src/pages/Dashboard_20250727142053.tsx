...
export function Dashboard() {
  // Estado para guardar o email do usuário logado
  const [userEmail, setUserEmail] = useState("Carregando...");

  // Exemplo de tarefas (pode substituir depois por dados dinâmicos)
  const tasks = [
    {
      id: 1,
  ...
  // useEffect para carregar email do usuário logado ao montar o componente
  useEffect(() => {
    async function fetchUserEmail() {
      try {
        const response = await api.get("/user/me"); // endpoint que retorna email do usuário logado
        setUserEmail(response.data.email);
      } catch (error) {
        console.error("Erro ao buscar email do usuário:", error);
        setUserEmail("Usuário não identificado");
      }
    }
    fetchUserEmail();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#192132] text-white p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <button className="bg-blue-600 p-3 rounded text-white font-semibold hover:bg-blue-700 transition flex items-center gap-2">
