...
const EditarAtividadeModal: React.FC<Props> = ({ isOpen, onClose, atividade, onAtividadeAtualizada }) => {
  ...
  const [descricao, setDescricao] = useState(atividade?.description || '');
  const [prioridade, setPrioridade] = useState(atividade?.priority || 'Baixa');
  const [status, setStatus] = useState(atividade?.status || 'Pendente');
  const [dataVencimento, setDataVencimento] = useState(atividade?.dueDate || '');
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`/tarefas/${atividade.id}`,
        { title: titulo, description: descricao, priority: prioridade, status });
